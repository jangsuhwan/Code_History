...
void Insert_Head(List* list, AData data) //맨 앞에 데이터 삽입하는 메서드
{
	if (list->numOfData == 0)
	{
...
void Find(List* list, AData data) //데이터 찾는 메서드
{
...
void Insert_Before(List* list, AData data1, AData data2) // data1 앞에 data2 삽입 하는 메서드
{
	int cur = list->curPosition;
	int arrTemp[SIZE];
...
void Insert_After(List* list, AData data1, AData data2) //data1 뒤에 data2 삽입
{
	int cur = list->curPosition;
	int arrTemp[SIZE];
...
void Replace(List* list, AData data1, AData data2) // data1를 data2로 교체
{
	int cur = list->curPosition;
...
void Erase(List* list, AData data) //data삭제
{
	int cur = list->curPosition;
...
void Next(List* list, AData data) // data 다음 data 출력
{
	int cur = list->curPosition;
	while (cur < (list->numOfData))
	{
...
void Previous(List* list, AData data) // data 이전 data 출력
{
	int cur = list->curPosition;
	while (cur < (list->numOfData))
	{
